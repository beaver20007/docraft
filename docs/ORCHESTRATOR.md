...
| H9 | **`ai-playbook#1`** (DRAFT, `cursor/dev-environment-setup-475d`, 31.05) — аудит базы чист: 1 свой коммит, 1 новый файл `AGENTS.md` (в `main` отсутствует → конфликта нет), но ветка отстала на **102 коммита**. Закрыть как устаревший (рекомендация Оркестратора) или дать воркеру освежить? | решение по чужому PR |
| H10 | **3 влитые ветки на origin суперрепо** — `chore/handoff-and-discovery`, `fix/ci-submodules`, `fix/gitmodules-desktop`; у всех уникальных коммитов **0**, содержимое целиком в `main`. Удалить для гигиены? | удаление веток на origin |
