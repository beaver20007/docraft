...
**Последнее обновление:** 2026-07-21 — влиты 4 трека (`pdf-001`, `quality-001`, `pdf-002`, `quality-002`),
запущен `fonts-001`, зафиксирована политика проверки качества (раздел 10).
...
| # | Репо | PR | Статус |
|---|---|---|---|
| 1 | ai-playbook | PR от **fonts-001** (`feat/ofl-font-bundle`) | воркер работает, PR ещё не создан |
...
| `fonts-001` — единый OFL-набор шрифтов в образ (Inter + OFL serif/mono) | **ai-playbook** (`render-engine` + `Dockerfile`) | 🔄 **В РАБОТЕ** с 2026-07-21 | — |
| `web-001` — UI не отличает «провалена» от «не выполнилась» (следствие `quality-002`) | ai-playbook (`apps/web`) | QUEUED — ТЗ готово, воркер не запущен | нет, можно запускать сразу |
